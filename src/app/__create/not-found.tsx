...
export default function CreateDefaultNotFoundPage() {
  ...
  // In SPA mode, loaderData is not available.
  // We'll mock missingPath or derive it from the current URL if necessary
  const location = useLocation();
  const missingPath = location.pathname.replace(/^\//, '');
  const existingRoutes: { path: string; url: string; }[] = []; // No longer dynamically fetched
